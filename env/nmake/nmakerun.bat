...
CALL "%VsPath%vsdevcmd.bat" && nmake -f %gmakefile% %gmake_cmd%
EXIT /B %ERRORLEVEL%
...
FOR /F "usebackq delims=" %%i in (`!VsWhereCmdLine!`) DO (
	...
	IF EXIST "%%i\VC\Auxiliary\Build\vcvars32.bat" (
		CALL "%%i\VC\Auxiliary\Build\vcvars32.bat" && nmake MSDEV="%PremakeVsVersion%" -f %gmakefile% %gmake_cmd%
		EXIT /B %ERRORLEVEL%
	) ELSE (
		IF EXIST "%%i\VC\Auxiliary\Build\vcvars64.bat" (
			CALL "%%i\VC\Auxiliary\Build\vcvars64.bat" && nmake MSDEV="%PremakeVsVersion%" -f %gmakefile% %gmake_cmd%
			EXIT /B %ERRORLEVEL%
		)
	)
)
